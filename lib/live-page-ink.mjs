...
function normalizeSendResult(value) {
  ...
  const tags = Array.isArray(value.tags) ? value.tags.slice(0, 24).map(tag => String(tag || "").slice(0, 64)) : [];
  const page = value.page && typeof value.page === "object" && !Array.isArray(value.page) ? {
    version: Number(value.page.version) || 0,
    title: String(value.page.title || "").slice(0, 240),
    revision: String(value.page.revision || "").slice(0, 80),
    updatedAt: String(value.page.updatedAt || "").slice(0, 80),
    interactive: value.page.interactive === true
  } : undefined;
  return {
    ok: true,
    ...
    ...(Object.prototype.hasOwnProperty.call(value, "intent") ? { intent: String(value.intent || "").slice(0, 64) } : {}),
    ...(Object.prototype.hasOwnProperty.call(value, "pageChanged") ? { pageChanged: value.pageChanged === true } : {}),
    ...(page ? { page } : {})
  };
}
...
export class LiveInkStore {
  ...
  async init() {
    ...
    // A pending claim belongs to the prior server process. It cannot still be
    // executing after startup, so retaining it only strands a browser retry.
    if ((this.state.sends || []).some(record => record.status === "pending")) {
      this.state.sends = this.state.sends.filter(record => record.status === "complete");
