...
export function renderLivingDocument(source, theme = "light") {
  ...
  const bridge = `<style id="live-viewer-theme">:root{--live-paper:${palette.paper}!important;--live-ink:${palette.ink}!important;--live-line:${palette.line}!important;--live-soft:${palette.soft}!important;color-scheme:${dark ? "dark" : "light"}}html,body{background:var(--live-paper)!important;color:var(--live-ink)!important}</style>`;
...
export function sanitizeLivingHtml(source) {
  ...
  const guard = `<meta http-equiv="Content-Security-Policy" content="${CSP}"><meta name="referrer" content="no-referrer">`;
...
export function normalizeLivingPage(input = {}) {
  ...
  const html = sanitizeLivingHtml(rawHtml);
  return {
    version: 2,
    title: safeTitle(rawHtml, input.title),
    html
  };
}
...
export class LivePageStore {
  ...
  validatePrepared(raw) {
    ...
    const content = normalizeLivingPage({ title: raw.title, html: raw.html });
    const revision = revisionFor(content);
