...
export async function atomicWriteFile(target, content, operations = fs) {
  const temp = `${target}.${process.pid}.${crypto.randomUUID()}.tmp`;
  let handle;
  try {
    handle = await operations.open(temp, "wx");
    ...
    await operations.unlink(temp).catch(() => {});
    throw error;
  }
}
...
export class WorkspaceStore {
  ...
  async init() {
    ...
    try {
      ...
      if (!Array.isArray(this.workspaces)) throw new Error("workspace index must be a JSON array");
    } catch (error) {
      if (error && error.code === "ENOENT") { this.workspaces = []; return; }
      const detail = error instanceof SyntaxError ? "workspace index is not valid JSON" : error.message;
      throw new Error(`Cannot load workspace index without risking data loss: ${detail}`);
    }
  }
  ...
  async saveInner() {
    ...
    try {
      ...
      await handle.writeFile(JSON.stringify(this.workspaces, null, 2), "utf8");
      await handle.sync();
      await handle.close(); handle = null;
      try { await fs.copyFile(this.indexPath, backup); }
  ...
  save() {
    const run = this.saveQueue.then(() => this.saveInner());
